...
public class ThreadWaitAndNotify {
    ...
    static class Thread0 extends Thread {
        ...
        public void run() {
            synchronized (object) {
                System.out.println(Thread.currentThread().getName() + "初次获得对象锁，执行中，接下来要调用共享对象的wait()方法...");
                try {
                    // 共享对象wait方法，会让线程释放锁。
                    object.wait();
    ...
    static class Thread1 extends Thread {
        ...
        public void run() {
            synchronized (object) {
                ...
                System.out.println(Thread.currentThread().getName() + "获得对象锁，执行中，接下来调用了共享对象的notify()方法");
                object.notify();
            }
        }
    }
